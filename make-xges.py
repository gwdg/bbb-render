...
class Presentation:
    ...
    def __init__(self, source, size):
        ...
        self.presentation_length = (
            float(self.xml_meta.find("./playback/duration").text.strip()) / 1000
        )
    ...
    def set_project_metadata(self, name, value):
        self.project.register_meta_string(GES.MetaFlag.READWRITE, name, value)
    ...
    def set_track_caps(self, fps=24, hz=48000):
        ...
        profile = GstPbutils.EncodingContainerProfile.new(
            "default", "bbb-render encoding profile", Gst.Caps.from_string("video/webm")
        )
        profile.add_profile(
            GstPbutils.EncodingVideoProfile.new(
                Gst.Caps.from_string("video/x-vp8"),
                "vp8_preset",
                self.video_track.props.restriction_caps,
                0,
            )
        )
        profile.add_profile(
            GstPbutils.EncodingAudioProfile.new(
                ...
                None,
                self.audio_track.props.restriction_caps,
                0,
            )
        )
    ...
    def _add_clip(self, layer, asset, *, ts, dt, pos, size, skip=0):
        ...
        logging.info(
            "Clip %s %r ts=%d dt=%d skip=%d pos=%r size=%r",
            layer,
            asset,
            ts,
            dt,
    ...
    def _total_length(self):
        return (
            self._opening_credits_length
            + self._cut[1]
            - self._cut[0]
            + self._closing_credits_length
        )
    ...
    def add_webcams(self, fit, align):
        ...
        self._add_clip(
            "Camera", video, ts=ts, dt=dt, skip=skip, pos=box[:2], size=box[2:]
        )
    ...
    def add_slides(self, fit, align):
        ...
        for png, start, duration in self._generate_slides(maxsize):
            ...
            self._add_clip(
                "Slides", png, ts=start, dt=duration, pos=box[:2], size=box[2:]
            )
    ...
    def _generate_slides(self, maxsize):
        ...
        for img in doc.iterfind("./{http://www.w3.org/2000/svg}image"):
            ...
            canvas = doc.find(
                './{{http://www.w3.org/2000/svg}}g[@class="canvas"][@image="{}"]'.format(
                    img.get("id")
                )
            )
            ...
            for shape_id, shapes in shapes.items():
                ...
                zindex = shapes[0][
                    0
                ]  # Use start time for z-layer ordering (new on top)
                ...
                for i, (start, undo, shape) in enumerate(shapes):
                    ...
                    if start >= img_end:
                        loging.warning(
                            "Shape timing is off: start=%d end=%s",
                            start / Gst.SECOND,
                            end / Gst.SECOND,
                        )
            ...
            for i, interval in enumerate(sorted(timeline)):
                ...
                png = self._render_slide(
                    [img] + shapes, size, f'{img.get("id")}-{i}.png'
                )
    ...
    def add_deskshare(self, fit, align):
        ...
        for event in events:
            ...
            self._add_clip(
                "Deskshare", video, ts=ts, skip=skip, dt=dt, pos=box[:2], size=box[2:]
            )
...
parser = argparse.ArgumentParser(
    description="convert a BigBlueButton presentation into a GES project"
)
parser.add_argument(
    "--size",
...
parser.add_argument(
    "--end", metavar="SECONDS", type=float, default=0, help="End point in the recording"
)
parser.add_argument(
    "--webcam-width",
    metavar="WIDTH",
...
parser.add_argument(
    "target", metavar="FILE", type=str, help="Output filename for GES project"
)
...
def main(argv):
    ...
    if cam_width > 0:
        ...
        p.add_webcams(
            fit=(slides_width + 2 * margin, margin, cam_width, max_height), align="lt"
        )
